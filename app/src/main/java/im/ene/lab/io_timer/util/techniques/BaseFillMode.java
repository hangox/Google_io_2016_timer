...
import im.ene.lab.io_timer.util.PathNodeUtils;
...
public class BaseFillMode extends AbstractFillMode {
  ...
  @Override public void fillInjectedNodes(ArrayList<PathParser.PathDataNode> from,
    ...
    for (int i = 0; i < from.size(); i++) {
      if (from.get(i).type == PathNodeUtils.CMD_PLACEHOLDER) {
      ...
      } else if (to.get(i).type == PathNodeUtils.CMD_PLACEHOLDER) {
      ...
      if (nodeMaster == null || nodePlaceholder == null) continue;

      nodePlaceholder.type = nodeMaster.type;
      nodePlaceholder.params =
          PathParser.copyOfRange(nodeMaster.params, 0, nodeMaster.params.length);
      ...
      if (Character.toLowerCase(nodeMaster.type) == 'z') {
        //Injecting a 'z' means we need to counterbalance the last path position with an extra 'm'
        PathParser.PathDataNode extraMoveNodePlaceholder =
            new PathParser.PathDataNode('M', new float[] { lastPlaceholderX, lastPlaceholderY });
        PathParser.PathDataNode extraMoveNodeMaster =
            new PathParser.PathDataNode('M', new float[] { lastMasterX, lastMasterY });
        ...
        penPosTo = PathNodeUtils.calculatePenPosition(to);
          ...
          nodePlaceholder.type)) { // this is a relative movement. If we want to create extra nodes, we need to create neutral relative commands
        Arrays.fill(nodePlaceholder.params, 0.0f); //FIXME is good?
      } else {
        if (nodePlaceholder.type == 'V') {
          nodePlaceholder.params[0] = lastPlaceholderY;
        } else if (nodePlaceholder.type == 'H') {
          nodePlaceholder.params[0] = lastPlaceholderX;
        } else {
          for (int j = 0; j < nodePlaceholder.params.length; j++) {
